  ...
  <UCard v-if="isVisible"
    ...
    <UForm :state="formState" class="space-y-3 md:space-y-4 px-4 md:px-6" @submit="onSubmit">
      <UFormGroup label="Прізвище" name="lastname">
        <UInput v-model="formState.lastname" placeholder="Введіть своє прізвище" color="yellow" variant="outline"
          size="md" class="rounded-full" />
      </UFormGroup>
      <UFormGroup label="Ім'я" name="firstname">
        <UInput v-model="formState.firstname" placeholder="Введіть своє ім'я" color="yellow" variant="outline" size="md"
          class="rounded-full" />
      </UFormGroup>
      <UFormGroup label="Вік" name="age">
        <UInput v-model="formState.age" type="number" placeholder="Введіть свій вік" color="yellow" variant="outline"
          size="md" class="rounded-full" />
      </UFormGroup>
      <UFormGroup label="Стать" name="gender">
        <USelect v-model="formState.gender" :options="genderOptions" placeholder="Виберіть свою стать" color="yellow"
          variant="outline" size="md" class="rounded-full" />
      </UFormGroup>
      <UFormGroup label="Education Level" name="educationLevel">
        <USelect v-model="formState.educationLevel" :options="educationOptions" placeholder="Виберіть рівень освіти"
          color="yellow" variant="outline" size="md" class="rounded-full" />
      </UFormGroup>
      <UFormGroup label="Мешканець з" name="residentSince">
        <USelect v-model="formState.residentSince" :options="residentOptions" placeholder="Як довго ви тут живете?"
          color="yellow" variant="outline" size="md" class="rounded-full" />
      </UFormGroup>
      <UFormGroup label="Житель Біля річки З" name="residentNearRiverSince">
        <UInput v-model="formState.residentNearRiverSince" placeholder="Введіть рік (наприклад, 2022)" color="yellow"
          variant="outline" size="md" class="rounded-full" />
      </UFormGroup>

      <div class="flex justify-center mt-6 mb-4">
        <UButton type="submit" class="rounded-full bg-sky-400 hover:bg-sky-500 text-white px-6 py-2 text-base">
          перейти до карти
        </UButton>
      </div>
    </UForm>
    ...
    <template #footer>
      <p class="text-xs text-gray-500 px-4 md:px-6 py-3 text-justify">
        Беручи участь в цьому опитуванні, ви даєте згоду на збір та використання ваших відповідей в дослідницьких цілях.
        Ваші персональні дані залишаться конфіденційними і не будуть передані третім особам без вашої згоди, за винятком
        випадків, передбачених законодавством. Однак будь-які надані вами медіафайли та коментарі будуть опубліковані.
        Участь у дослідженні є добровільною, і ви можете відмовитися від неї в будь-який час.
      </p>
    </template>
  </UCard>
</template>
