...
<template>
  <div class="px-4 bg-[#004E32] overflow-auto" :style="{ height: 'calc(100vh - 50px)' }">
    <div class="bg-[#41715C] pb-4 px-4">
      ...
      <div class="flex overflow-auto gap-4">
        <div v-for="name in hubsList" :key="name">
          <OverviewCard class="border-4 cursor-pointer" :class="existingHubs[name] ? 'border-[#DBFF00]' : 'border-[#41715C]'
    " :name="name" @click="existingHubs[name] = !existingHubs[name]" />
        </div>
      </div>
    </div>

    <div class="mt-8">
      <div v-for="metric in selectedDatasets" :key="metric" class="bg-[#41715C] overflow-y-auto w-full mb-8">
        <h1 class="p-4">{{ metric }}</h1>
        <DataCard :chart-data="chartData[metric] !== undefined ? chartData[metric] : {}" />
      </div>
    </div>
  </div>
</template>
