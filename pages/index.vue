...
<template>
  <div class="relative">
    ...
    <GeneralizedHeader class="z-20" :left-items="leftItems" logo-src="/neu-logo.svg"
      logo-alt="Northeastern University Logo" :show-icon="true" />
    <GeneralizedFooter />
    <RegistrationPopup :is-visible="mapUIStore.showRegistration" @close="mapUIStore.showRegistration = false" />
    <div v-if="isMapBlurred" class="absolute inset-0 bg-black bg-opacity-50 z-40"
      @click.self="mapUIStore.showRegistration = true"></div>
  </div>
</template>
<style scoped>
.blur-md {
